...
inline void compute_generic(int n_samples, int l_max, DTYPE *prefactors,
                            ...
                            DTYPE *buffers) {
  if (dsph == nullptr) {
    generic_sph<DTYPE, false, false, false, 1>(xyz, sph, dsph, ddsph, n_samples,
                                               l_max, prefactors, buffers);
  } else if (ddsph == nullptr) {
    generic_sph<DTYPE, true, false, false, 1>(xyz, sph, dsph, ddsph, n_samples,
                                              l_max, prefactors, buffers);
  } else {
    generic_sph<DTYPE, true, true, false, 1>(xyz, sph, dsph, ddsph, n_samples,
                                             l_max, prefactors, buffers);
  }
}
...
void run_timings(int l_max, int n_tries, int n_samples) {
  ...
  benchmark(
      "Call without derivatives (no hardcoding)", n_samples, n_tries, [&]() {
        compute_generic<DTYPE>(n_samples, l_max, prefactors.data(), xyz.data(),
...

  {
    ...
    // hence this is equivalent to the first call
    benchmark(
        "Call with second derivatives (normalized)", n_samples, n_tries,
        [&]() { calculator.compute_with_hessians(xyz, sph1, dsph1, ddsph1); });
  }
  std::cout << std::endl;
  ...
  benchmark("L=1 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 1>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=1 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 1>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
  ...
  benchmark("L=2 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 2>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=2 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 2>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
  ...
  benchmark("L=3 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 3>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=3 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 3>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
  ...
  benchmark("L=4 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 4>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=4 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 4>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
  ...
  benchmark("L=5 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 5>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=5 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 5>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
  ...
  benchmark("L=6 hardcoded values            ", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, false, false, false, 6>(xyz.data(), sph1.data(),
                                                 nullptr, nullptr, n_samples, 0,
                                                 nullptr, nullptr);
  });

  benchmark("L=6 hardcoded values+derivatives", n_samples, n_tries, [&]() {
    hardcoded_sph<DTYPE, true, false, false, 6>(xyz.data(), sph1.data(),
                                                dsph1.data(), nullptr,
                                                n_samples, 0, nullptr, nullptr);
  });
  std::cout << std::endl;
...
int main(int argc, char *argv[]) {
  ...
  std::cout << "Running with n_tries=" << n_tries << ", n_samples=" << n_samples
            << std::endl;
  std::cout << "\n============= l_max = " << l_max
            << " ==============" << std::endl;
