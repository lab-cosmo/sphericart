...
public:
   ...
   * evaluated on the unit sphere. In practice, this simply computes the scaled
   * harmonics at the normalized coordinates \f$(x/r, y/r, z/r)\f$, and adapts
   ...
   *        It is a (possibly uninitialized) `std::vector`, which, if needed,
   *        will be resized to `n_samples * 3 * 3 * (l_max + 1) * (l_max + 1)`.
   * On exit, this array will contain the second derivatives of the spherical
   *        harmonics organized along four dimensions. As for the `sph`
   ...
   * derivatives organized along two dimensions. The second dimension's size is
   * `(l_max + 1) * (l_max + 1)`, and it represents the degree and order of the
   * spherical harmonics (again, organized in lexicographic order). The first
   * dimension corresponds to the different spatial derivatives of the
...
private:
  ...
  void (*_array_with_derivatives)(const T *, T *, T *, T *, size_t, int,
                                  const T *, T *);
  void (*_array_with_hessians)(const T *, T *, T *, T *, size_t, int, const T *,
                               T *);

  // these compute a single sample
  void (*_sample_no_derivatives)(const T *, T *, T *, T *, int, int, const T *,
                                 const T *, T *, T *, T *);
