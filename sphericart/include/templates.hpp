...
inline void hardcoded_sph_sample(const T *xyz_i, T *sph_i,
  ...
  */
  ...
  if constexpr (DO_DERIVATIVES) {
    ...
    HARDCODED_SPH_DERIVATIVE_MACRO(HARDCODED_LMAX, x, y, z, x2, y2, z2, sph_i,
                                   dx_sph_i, dy_sph_i, dz_sph_i, DUMMY_SPH_IDX);
    ...
    if constexpr (DO_SECOND_DERIVATIVES) {
      ...
      if constexpr (NORMALIZED) {
        for (int k = 0; k < size_y; ++k) {
          ...
          // derivatives) to avoid crazy nesting of these sections. The
          // main issue is that if(constexpr) restricts the scope of
          // the double derivative pointers.

          // correct second derivatives for normalization. We do it
          // before the first derivatives because we need the unchanged
          // first derivatives
          auto irsq = ir * ir;
          ...
          dxdx_sph_i[k] = (-2 * x * tmpx + dxdx_sph_i[k] + 3 * x * x * tmp -
                           tmp - 2 * x * dx_sph_i[k] + x * x * tmp2) *
                          irsq;
          dydy_sph_i[k] = (-2 * y * tmpy + dydy_sph_i[k] + 3 * y * y * tmp -
                           tmp - 2 * y * dy_sph_i[k] + y * y * tmp2) *
                          irsq;
          dzdz_sph_i[k] = (-2 * z * tmpz + dzdz_sph_i[k] + 3 * z * z * tmp -
                           tmp - 2 * z * dz_sph_i[k] + z * z * tmp2) *
                          irsq;
...
#pragma omp for
    for (size_t i_sample = 0; i_sample < n_samples; i_sample++) {
      ...
      hardcoded_sph_sample<T, DO_DERIVATIVES, DO_SECOND_DERIVATIVES, NORMALIZED,
                           HARDCODED_LMAX>(xyz_i, sph_i, dsph_i, ddsph_i,
                                           HARDCODED_LMAX, size_y);
    }
  }
}
  ...
  for (auto m = l - 2; m > HARDCODED_LMAX - 1; --m) {
    ...
    if constexpr (DO_DERIVATIVES) {
      ...
      if constexpr (DO_SECOND_DERIVATIVES) {
        ...
        ql2m_2 = ql2m_1;
        ql2m_1 = ql2m_0; // shift at the end because we need all three at
                         // the same time
      }
    }
  }
  ...
  if constexpr (DO_DERIVATIVES) {
    ...
    if constexpr (DO_SECOND_DERIVATIVES) {
      if (l == 2) {
        ...
        ql2m_0 =
            qlmk[-2 * l + 1] * (twomz[GET_INDEX(0)] * ql2m_1 + rxy * ql2m_2);
      }
...
generic_sph_sample(const T *xyz_i, T *sph_i, [[maybe_unused]] T *dsph_i,
  ...
  /*
  This is a low-level function that combines all the pieces to evaluate the sph
  for a single sample. It calls both the hardcoded macros and the generic
  ...
  */
  ...
  for (int l = HARDCODED_LMAX + 1; l < l_max + 1; l++) {
    generic_sph_l_channel<T, DO_DERIVATIVES, DO_SECOND_DERIVATIVES,
        ...
        dz_sph_i, dxdx_sph_i, dxdy_sph_i, dxdz_sph_i, dydx_sph_i, dydy_sph_i,
        dydz_sph_i, dzdx_sph_i, dzdy_sph_i, dzdz_sph_i);
...
void generic_sph(const T *xyz, T *sph, [[maybe_unused]] T *dsph,
  ...
  /*
     ...
     dYlm/dx,dy,dz. stored as for sph_i, with three consecutive blocks
     associated to d/dx,d/dy,d/dz
      [[maybe_unused]] T *dsph : pointer to the storage location for the second
     derivatives. stored as for sph_i, with nine consecutive blocks associated
     to the possible second derivative combinations size_t n_samples: number of
     samples that have to be computed int l_max: maximum l to compute
      prefactors: pointer to an array that contains the prefactors used for Ylm
     and Qlm calculation buffers: buffer space to compute cosine, sine and
     2*m*z terms
  */
...
#pragma omp for
    for (size_t i_sample = 0; i_sample < n_samples; i_sample++) {
      ...
      generic_sph_sample<T, DO_DERIVATIVES, DO_SECOND_DERIVATIVES, NORMALIZED,
                         HARDCODED_LMAX>(xyz_i, sph_i, dsph_i, ddsph_i, l_max,
                                         size_y, prefactors, qlmfactors, c, s,
                                         twomz);
    }
  }
}
