...
//    // calculation examples
    ...
    calculator_sycl.compute_with_gradients(xyz_device, n_samples, sph_device, dsph_device ); // compute with gradients in device
    // Get results from device to host
    DEVICE_GET(DTYPE, sph_f.data(), sph_device, sph_f.size())
    ...
    DTYPE size2 = (l_max + 1) * (l_max + 1); // Size of the second+third dimensions in derivative arrays
    DTYPE sph_error = 0.0, sph_norm = 0.0, sph_error_h = 0.0, sph_error_h_max = 0.0;
