...
inline void _compute_sph_prefactors(int l_max, yq_buffer<DTYPE> *factors) {
    /*
    ...
    */
    ...
    for (int l = 0; l <= l_max; ++l) {
        ...
        factors[k].y = sqrt(factor) * M_SQRT1_2;        
        for (int m = 1; m <= l; ++m) {
            factor *= 1.0 / (l * (l + 1) + m * (1 - m));
            if (m % 2 == 0) {
                factors[k + m].y = sqrt(factor);
            } else {
                factors[k + m].y = -sqrt(factor);
            }
        }
    ...
    k=0; factors[k].q = 1.0; k += 1;
    for (int l = 1; l < l_max + 1; l++) {
        factors[k+l].q = -(2 * l - 1) * factors[k - 1].q;
        for (int m = l - 1; m >= 0; --m) {
            factors[k + m].q = -1.0 / ((l + m + 1) * (l - m));
        }        
        k += l + 1;
    }
}
...
void compute_sph_prefactors(int l_max, yq_buffer<double> *factors) {
...
void cartesian_spherical_harmonics(int n_samples,
    int l_max, const yq_buffer<double> *prefactors,
...
void normalized_spherical_harmonics(
...
void cartesian_spherical_harmonics_sample(int l_max,
    ...
    generic_sph_sample<double, true, false, SPHERICART_LMAX_HARDCODED>(l_max,  (l_max + 1) * (l_max + 1), prefactors, buffer, xyz_i, sph_i, dsph_i);
}
} // namespace sphericart