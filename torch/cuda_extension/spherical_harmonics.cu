...
*/
__device__ int get_index_(int i, int buff_size)
{
    return threadIdx.y * buff_size + i * blockDim.x + threadIdx.x;
}

/*
Computes the index for buffer values which are shared across GRID_DIM_Y
*/
__device__ int get_index(int i)
{
    return i * blockDim.x + threadIdx.x;
}
template <typename scalar_t>
__device__ void compute_sph_l0(scalar_t *sph)
{
...
template <typename scalar_t>
__device__ void compute_sph_l1(scalar_t x, scalar_t y, scalar_t z, scalar_t *sph)
{
...
template <typename scalar_t>
__device__ void compute_dsph_l2(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph_i, scalar_t *dxsph_i, scalar_t *dysph_i, scalar_t *dzsph_i)
{
...
template <typename scalar_t>
__device__ void compute_dsph_l3(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph_i, scalar_t *dxsph_i, scalar_t *dysph_i, scalar_t *dzsph_i)
{
...
__device__ void compute_sph_l4(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph)
{
    ...
    sph[get_index(20)] = -0.69436507482941 * (y * sph[get_index(11)] - 1.6329931618554521 * z * sph[get_index(12)] + x * sph[get_index(13)]);
...
template <typename scalar_t>
__device__ void compute_dsph_l4(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph_i, scalar_t *dxsph_i, scalar_t *dysph_i, scalar_t *dzsph_i)
{
    ...
    dxsph_i[get_index(21)] = -1.338093087114578 * (z * z2 - 2.745873698591307 * y * sph_i[get_index(5)] - 4.019547514144073 * sph_i[get_index(12)]);
...
template <typename scalar_t>
__device__ void compute_dsph_l5(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph_i, scalar_t *dxsph_i, scalar_t *dysph_i, scalar_t *dzsph_i)
{
    dxsph_i[get_index(25)] = 5.244044240850758 * sph_i[get_index(16)];
    dxsph_i[get_index(26)] = 4.690415759823430 * sph_i[get_index(17)];
    dxsph_i[get_index(27)] = 3.582364210034113 * (y2 * sph_i[get_index(4)] + 3.58568582800318 * x * sph_i[get_index(11)]);
    dxsph_i[get_index(28)] = -8.774964387392122 * ((y2 - z2) * sph_i[get_index(5)] + 0.3086066999241838 * sph_i[get_index(17)]);
    dxsph_i[get_index(29)] = -1.914854215512676 * sph_i[get_index(18)];
    dxsph_i[get_index(30)] = -3.496029493900505 * sph_i[get_index(21)];
    dxsph_i[get_index(31)] = -8.616843969807043 * (0.2102610435016800 * z2 * z2 +
                                                   1.056887279361603 * sph_i[get_index(5)] * sph_i[get_index(5)] + (y2 - z2) * sph_i[get_index(6)] + 0.555555555555556 * sph_i[get_index(22)]);
    dxsph_i[get_index(32)] = -8.774964387392122 * (x2 - z2) * sph_i[get_index(7)];
    dxsph_i[get_index(33)] = -5.170697352496190 * (0.106904496764970 * z * dxsph_i[get_index(23)] -
                                                   0.320713490294909 * y * sph_i[get_index(9)] - sph_i[get_index(22)]);
    ...
    dysph_i[get_index(27)] = -3.102418411497714 * (0.534522483824849 * y * sph_i[get_index(9)] - 0.654653670707977 * z * sph_i[get_index(14)] - sph_i[get_index(22)]);
    dysph_i[get_index(28)] = -8.77496438739212 * (y2 - 1.585330919042404 * sph_i[get_index(6)]) * sph_i[get_index(7)];
    dysph_i[get_index(29)] = 0.7237468644557459 * (y * (2.12132034355964 * sph_i[get_index(9)] - 8.21583836257749 * sph_i[get_index(11)]) + 6.70820393249937 * z * sph_i[get_index(12)] + sph_i[get_index(24)]);
    ...
    dysph_i[get_index(33)] = 3.582364210034113 * sph_i[get_index(4)] * (y2 - 5 * z2 - 1.585330919042404 * sph_i[get_index(6)]);
...
__device__ void compute_sph_l6(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph)
{
    ...
    sph[get_index(42)] = -1.087114613009218 *
                         (0.645497224367903 * y * sph[get_index(29)] - z * sph[get_index(30)] + 0.645497224367903 * x * sph[get_index(31)]);
    sph[get_index(44)] = -0.9414688716912718 * (y * sph[get_index(27)] - 1.63299316185545 * z * sph[get_index(32)] + x * sph[get_index(33)]);
    sph[get_index(48)] = -1.040832999733066 * (y * sph[get_index(25)] - x * sph[get_index(35)]);
}

template <typename scalar_t>
__device__ void compute_dsph_l6(scalar_t x, scalar_t y, scalar_t z, scalar_t x2, scalar_t y2, scalar_t z2, scalar_t *sph_i, scalar_t *dxsph_i, scalar_t *dysph_i, scalar_t *dzsph_i)
{
    scalar_t tmp;
    ...
    dxsph_i[get_index(38)] = -8.07303841165959 * y *
                             (y2 * y2 - 4.188790204786391 * sph_i[get_index(5)] * sph_i[get_index(5)] - 2.642218198404007 * sph_i[get_index(22)]);
    dxsph_i[get_index(39)] = -15.29705854077835 * ((y2 - z2) * sph_i[get_index(10)] + 0.2611164839335468 * sph_i[get_index(26)]);
    dxsph_i[get_index(40)] =
        32.08092506951781 * (sph_i[get_index(5)] * (0.577350269189626 * y * sph_i[get_index(5)] - z * sph_i[get_index(6)]) +
    ...
    dxsph_i[get_index(43)] = 4.660970900149851 * (z2 * z * (1.666666666666667 * y2 + z2 - 2.642218198404007 * sph_i[get_index(6)]) +
    ...
    dxsph_i[get_index(45)] = 3.122498999199199 *
                             (y * sph_i[get_index(17)] - 1.224744871391589 * z2 * sph_i[get_index(14)] + +1.846372364689991 * sph_i[get_index(32)]);
    ...
    dysph_i[get_index(39)] = -3.122498999199199 *
                             (-1.22474487139159 * z2 * sph_i[get_index(14)] + y * sph_i[get_index(17)] - 1.10782341881399 * sph_i[get_index(32)]);
    dysph_i[get_index(40)] =
        11.68332144554792 * (x * (-1.585330919042404 * sph_i[get_index(5)] * sph_i[get_index(5)] + (z2 - y2) * sph_i[get_index(6)]) +
                             0.1740776559556978 * sph_i[get_index(31)]);
    dysph_i[get_index(41)] =
        -6.99145635022478 * z * (z2 * z2 + (5.28443639680801 * y2 - 4.188790204786391 * sph_i[get_index(6)]) * sph_i[get_index(6)]);
    ...
    dysph_i[get_index(46)] = 11.40175425099138 * (0.2360174359706574 * y2 * y2 * y + (y2 - 3 * z2) * sph_i[get_index(9)] +
...
template <typename scalar_t>
__device__ void generic_sph_l_channel_device(int l, scalar_t x, scalar_t y, scalar_t z, scalar_t rxy, scalar_t twoz, scalar_t *sph,
                                             scalar_t *dsph_x, scalar_t *dsph_y, scalar_t *dsph_z, int sph_offset, scalar_t *pk, scalar_t *qlmk, scalar_t *c, scalar_t *s, bool requires_grad)
{
...
template <typename scalar_t>
__device__ inline void clear_buffers(int nelements, scalar_t *sph, scalar_t *dsph_x, scalar_t *dsph_y, scalar_t *dsph_z, bool requires_grad)
{
...
template <typename scalar_t>
__device__ inline void write_buffers(int atom_idx, int natoms, scalar_t x, scalar_t y, scalar_t z, scalar_t ir, int n_elements, int offset,
                                     scalar_t *buffer_sph, scalar_t *buffer_dsph_x, scalar_t *buffer_dsph_y, scalar_t *buffer_dsph_z,
                                     torch::PackedTensorAccessor32<scalar_t, 2, torch::RestrictPtrTraits> sph,
                                     torch::PackedTensorAccessor32<scalar_t, 3, torch::RestrictPtrTraits> dsph, bool requires_grad,
                                     bool normalize)
{
    if (atom_idx < natoms)
    {
                                                   ...
                                                   torch::PackedTensorAccessor32<scalar_t, 3, torch::RestrictPtrTraits> dsph)
{
    ...
    clear_buffers((HARDCODED_LMAX + 1) * (HARDCODED_LMAX + 1), buffer_sph, buffer_dsph_x, buffer_dsph_y, buffer_dsph_z, requires_grad);
...
void adjust_shared_memory(torch::Tensor xyz, int lmax, int GRID_DIM_Y, bool requires_grad)
{
    ...
    TORCH_CHECK(total_buff_size <= deviceProp.sharedMemPerBlock * 4, "requested shared memory buffer (",
                total_buff_size, ") exceeds max available on device: ", deviceProp.name, " (", deviceProp.sharedMemPerBlock * 4, ")");

    switch (xyz.type().scalarType())
    {
    case torch::ScalarType::Double:
        cudaFuncSetAttribute(generic_spherical_harmonics_kernel<double>, cudaFuncAttributeMaxDynamicSharedMemorySize, total_buff_size);
    case torch::ScalarType::Float:
        cudaFuncSetAttribute(generic_spherical_harmonics_kernel<float>, cudaFuncAttributeMaxDynamicSharedMemorySize, total_buff_size);
    //case torch::ScalarType::Half:
     //   cudaFuncSetAttribute(generic_spherical_harmonics_kernel<at::Half>, cudaFuncAttributeMaxDynamicSharedMemorySize, total_buff_size);
    }
}
                                                           ...
                                                           int GRID_DIM_Y)
{
    ...
    auto sph = torch::zeros({ntotal, xyz.size(0)},
                            torch::TensorOptions()
                                .dtype(xyz.dtype())
                                .device(xyz.device()));
    ...
    {
        d_sph = torch::zeros({ntotal, 3, xyz.size(0)},
                             torch::TensorOptions()
                                 .dtype(xyz.dtype())
                                 .device(xyz.device()));
    }
    ...
    {
        d_sph = torch::zeros({1, 1, 1},
                             torch::TensorOptions()
                                 .dtype(xyz.dtype())
                                 .device(xyz.device())); // just so accessor doesn't complain
    }
    ...
    // to make computing hardcoded + generic easier, use a minimum amount of shared memory enough to store all hardcoded derivatives
    int nl = max((HARDCODED_LMAX + 1) * (HARDCODED_LMAX + 1), 2 * lmax + 1);

    AT_DISPATCH_FLOATING_TYPES(xyz.scalar_type(), "sparse_accumulation_forward_cuda", ([&]
                                                                                       {
                                                                                           size_t total_buff_size = 0;
...
{
public:
    static torch::Tensor forward(torch::autograd::AutogradContext *ctx, int lmax, torch::Tensor prefactors, torch::Tensor xyz)
    {
...
{
    m.def("adjust_shared_memory", &adjust_shared_memory, "check and change the allocation of shared memory to fit requirement");
    m.def("spherical_harmonics_cuda", &spherical_harmonics_cuda, "forwards computation for spherical harmonics (CUDA)");
    m.def("_generic_spherical_harmonics_gpu", &generic_spherical_harmonics_gpu, "generic function for spherical harmonics (CUDA)");
}