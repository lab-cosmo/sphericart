// This file defines the Python interface to the XLA custom calls implemented on the CPU.
// It is exposed as a standard pybind11 module defining "capsule" objects containing our
// methods. For simplicity, we export a separate capsule for each supported dtype.
...
void cpu_sph_with_hessians(void *out_tuple, const void **in) {
    ...
    auto calculator = sphericart::SphericalHarmonics<T>(l_max, normalized);
    calculator.compute_array_with_hessians(xyz, xyz_length, sph, sph_len, dsph, dsph_len, ddsph, ddsph_len);
}
