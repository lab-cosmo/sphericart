...
require the user to provide `std::vector`s to store the results. Since these `std::vector`s
...
illustrated in the example below. See the :doc:`API documentation <api>` for alternative calls
