...
__device__ inline void write_buffers(
    size_t atom_idx, size_t natoms, scalar_t x, scalar_t y, scalar_t z,
    ...
    bool requires_grad, bool requires_hessian, bool normalize) {
  if (atom_idx < natoms) {
      ...
      if (requires_hessian) {
        ...
        if (normalize) {
          ...
          auto tmp2 = x * x * tmp_dxdx + y * y * tmp_dydy + z * z * tmp_dzdz +
                      2 * x * y * tmp_dxdy + 2 * x * z * tmp_dxdz +
                      2 * y * z * tmp_dydz;

          tmp_dxdx = (-2 * x * tmpx + tmp_dxdx + 3 * x * x * tmp - tmp -
                      2 * x * tmp_dx + x * x * tmp2) *
                     (ir * ir);
          tmp_dydy = (-2 * y * tmpy + tmp_dydy + 3 * y * y * tmp - tmp -
                      2 * y * tmp_dy + y * y * tmp2) *
                     (ir * ir);
          tmp_dzdz = (-2 * z * tmpz + tmp_dzdz + 3 * z * z * tmp - tmp -
                      2 * z * tmp_dz + z * z * tmp2) *
                     (ir * ir);

          tmp_dxdy = tmp_dydx =
              (-x * tmpy - y * tmpx + tmp_dxdy + 3 * x * y * tmp - x * tmp_dy -
               y * tmp_dx + x * y * tmp2) *
              (ir * ir);
          tmp_dxdz = tmp_dzdx =
              (-x * tmpz - z * tmpx + tmp_dxdz + 3 * x * z * tmp - x * tmp_dz -
               z * tmp_dx + x * z * tmp2) *
              (ir * ir);
          tmp_dzdy = tmp_dydz =
              (-z * tmpy - y * tmpz + tmp_dzdy + 3 * y * z * tmp - z * tmp_dy -
               y * tmp_dz + y * z * tmp2) *
              (ir * ir);
        }
...
__global__ void spherical_harmonics_kernel(
    ...
    torch::PackedTensorAccessor64<scalar_t, 1, torch::RestrictPtrTraits>
        prefactors,
    int lmax, bool requires_grad, bool requires_hessian, bool normalize,
    torch::PackedTensorAccessor64<scalar_t, 2, torch::RestrictPtrTraits> sph,
    torch::PackedTensorAccessor64<scalar_t, 3, torch::RestrictPtrTraits> dsph,
  ...
  if (threadIdx.x == 0) {
    ...
    if (lmax >= 1) {
      ...
      if (requires_grad) {
        HARDCODED_SPH_DERIVATIVE_MACRO(1, x, y, z, x2, y2, z2, buffer_sph,
                                       buffer_dsph_x, buffer_dsph_y,
                                       buffer_dsph_z, get_index);
      }

      if (requires_hessian) {
        HARDCODED_SPH_SECOND_DERIVATIVE_MACRO(
            1, buffer_sph, buffer_dsph_dxdx, buffer_dsph_dxdy, buffer_dsph_dxdz,
            buffer_dsph_dydx, buffer_dsph_dydy, buffer_dsph_dydz,
            buffer_dsph_dzdx, buffer_dsph_dzdy, buffer_dsph_dzdz, get_index);
      }
    } else {
      COMPUTE_SPH_L0(buffer_sph, get_index);
      if (requires_grad) {
        COMPUTE_SPH_DERIVATIVE_L0(buffer_sph, buffer_dsph_x, buffer_dsph_y,
                                  buffer_dsph_z, get_index);

        if (requires_hessian) {
          COMPUTE_SPH_SECOND_DERIVATIVE_L0(
              buffer_sph, buffer_dsph_dxdx, buffer_dsph_dxdy, buffer_dsph_dxdz,
              buffer_dsph_dydx, buffer_dsph_dydy, buffer_dsph_dydz,
              buffer_dsph_dzdx, buffer_dsph_dzdy, buffer_dsph_dzdz, get_index);
        }
      }
  ...
  // global memory.
  write_buffers(atom_idx, natoms, x, y, z, ir, (ml + 1) * (ml + 1), 0,
                ...
                ddsph, requires_grad, requires_hessian, normalize);
  ...
  for (int l = HARDCODED_LMAX + 1; l < lmax + 1; l += 1) {
    ...
    /*
        sph_offset needs to point to Y[l, 0], so the mapping from array indices
       to memory locations may look like: sph 0: 0, sph_offset: 0 sph 1: 0 1 2,
       sph_offset: 1 sph 2: 0 1 2 3 4, sph_offset: 2 sph 3: 0 1 2 3 4 5 6,
       sph_offset: 3 we also need to make sure we select the right atom in the
       buffer, hence multiplication by blockDim.y.
    */
    ...
    write_buffers(atom_idx, natoms, x, y, z, ir, 2 * l + 1, base_index,
                  buffer_sph, buffer_dsph_x, buffer_dsph_y, buffer_dsph_z,
                  buffer_dsph_dxdx, buffer_dsph_dxdy, buffer_dsph_dxdz,
                  buffer_dsph_dydx, buffer_dsph_dydy, buffer_dsph_dydz,
                  buffer_dsph_dzdx, buffer_dsph_dzdy, buffer_dsph_dzdz, sph,
                  dsph, ddsph, requires_grad, requires_hessian, normalize);
...
static size_t total_buffer_size(size_t l_max, size_t GRID_DIM_X,
                                ...
                                bool requires_grad, bool requires_hessian) {
  int nl = max(static_cast<size_t>((HARDCODED_LMAX + 1) * (HARDCODED_LMAX + 1)),
               2 * l_max + 1);
  ...
  AT_DISPATCH_FLOATING_TYPES(
      xyz.scalar_type(), "spherical_harmonics_cuda", ([&] {
        size_t total_buff_size = total_buffer_size(
            ...
            xyz.requires_grad() || gradients, xyz.requires_grad() && hessian);
        ...
        spherical_harmonics_kernel<<<block_dim, grid_dim, total_buff_size>>>(
            ...
            l_max, xyz.requires_grad() || gradients,
...
torch::Tensor sphericart_torch::prefactors_cuda(int64_t l_max,
                                                at::ScalarType dtype) {
  auto result = torch::empty({(l_max + 1) * (l_max + 2)},
