...
SphericalHarmonics::compute_raw_cpu(torch::Tensor xyz, bool do_gradients,
  ...
  if (xyz.dtype() == c10::kDouble) {
    if (do_hessians) {
      auto dsph =
          torch::empty({n_samples, 3, (l_max_ + 1) * (l_max_ + 1)}, options);
      auto ddsph =
          torch::empty({n_samples, 3, 3, (l_max_ + 1) * (l_max_ + 1)}, options);
      ...
      calculator_double_.compute_array(xyz.data_ptr<double>(), n_samples * 3,
                                       sph.data_ptr<double>(), sph_length);
    ...
    if (do_hessians) {
      ...
      calculator_float_.compute_array(xyz.data_ptr<float>(), n_samples * 3,
  ...
  if (xyz.device().is_cpu()) {
    ...
    if (!shm_result) {
      ...
      printf("element_size = %ld, GRID_DIM_X = %ld, GRID_DIM_Y = %ld, "
             ...
             "xyz.requires_grad() || "
             "do_hessians = %s\n",
             torch::elementSize(xyz.scalar_type()), calculator.CUDA_GRID_DIM_X_,
...
torch::autograd::variable_list SphericalHarmonicsAutogradBackward::backward(
  ...
  if (xyz.requires_grad()) {
    // gradgrad_wrt_xyz is needed to differentiate twice with respect to xyz.
    // However, we only do this if the user requested it when creating the
    // class
    if (double_backward) {
    ...
    // if double_backward is false, xyz requires a gradient, but the user did
    // not request second derivatives with respect to xyz (and therefore
